...
object Scheduler {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val result = (0 until machines) map { _ =>
      ...
      jobIndices.reverse map (sortedJobs.remove(_)) // remove scheduled jobs
      scheduledJobs
    }
   ...
   * @returns the indices of the scheduled jobs
   */
   ...
   * @returns the index of the Job or -1 if there is none
   */
   ...
   * @returns the indices of the jobs that form the optimal solution 
   */
